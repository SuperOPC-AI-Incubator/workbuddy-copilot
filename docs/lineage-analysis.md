...
2. **同事 TanStack/Supabase 仓库是用户确认的新产品主线。** 双角色 Web、Supabase Auth/RLS/Realtime、MCP/连接器和云端部署均在这条主线继续演进；这是一项产品与架构选择，不是 Git 血缘分析自动得出的结论。
3. **原 Python 仓库是需求、可靠性与测试资产供体。** 可复用的是完整对话、幂等、补传、离线恢复、真实送达、诊断上下文和防假绿方法；不要求保留 Python/FastAPI/SQLite 的具体实现，也不把 `copilot.db`、单 worker、旧 hook 形态等旧架构约束带入新主线。
4. **按用户路径和失败语义吸收能力。** 每个能力以 API 契约、状态机、负控和端到端结果为迁移单位，在 TanStack/Supabase 主线中实现并验证；不按旧目录复制，也不用文件相似度代替行为验证。
5. **保留来源与决策记录。** 采用任一仓库中的界面、文案、协议或实现时，应链接对应提交和设计决策，明确“产品来源”“实现来源”和“当前主线”，避免再次把语义复刻误判为同一 Git 分支。
